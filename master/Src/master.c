...
int16_t target_pos_z = 0;
uint8_t slave_addr_y = 0x10; // Slave I2C addr
uint8_t slave_addr_z = 0x12;
...
int main(void)
{
 ...
 while (1)
 {
   ...
   if (x_val > threshold || x_val < -threshold) {
     ...
     if (target_position > xMax) {
      target_position = xMax;
     }
     else if (target_position < xMin) {
      target_position = xMin;
     }
   }
   if (y_val > threshold || y_val < -threshold) {
     target_pos_y += (y_val / 16);
     if (target_pos_y > yMax) {
      target_pos_y = yMax;
     }
     else if (target_pos_y < yMin) {
      target_pos_y = yMin;
     }
   }
   if (z_val > threshold || z_val < -threshold) {
     target_pos_z += (z_val / 16);
     if (target_pos_z > zMax) {
      target_pos_z = zMax;
     }
     else if (target_pos_z < zMin) {
      target_pos_z = zMin;
     }
   }
   ...
   // Pack & Transmit y-axis to slave1
   char tx_buff_y[2];
   tx_buff_y[0] = (char)(target_pos_y & 0xFF); // lo byte
   tx_buff_y[1] = (char)((target_pos_y >> 8) & 0xFF); // hi
   ...
   // Pack & Transmit z-axis to slave2
   char tx_buff_z[2];
   tx_buff_z[0] = (char)(target_pos_z & 0xFF);
   tx_buff_z[1] = (char)((target_pos_z >> 8) & 0xFF);
